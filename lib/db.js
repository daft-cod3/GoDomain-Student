...
const PROJECT_ROOT = path.normalize(path.resolve(__dirname, ".."));
const CA_CERT_PATH = path.normalize(path.resolve(__dirname, "..", "cert", "prod-ca-2021.crt"));

if (!CA_CERT_PATH.startsWith(PROJECT_ROOT + path.sep)) {
  throw new Error("CA cert path escapes project root");
}

function readCaCert() {
  const certPath = path.normalize(path.resolve(__dirname, "..", "cert", "prod-ca-2021.crt"));
  if (!certPath.startsWith(PROJECT_ROOT + path.sep)) {
    throw new Error("CA cert path escapes project root");
  }
